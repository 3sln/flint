...
| [0035](0035-reader-tags.md) | A reader tag is a name; the var it names is the identity | **Queued.** An unknown tag in source is now an ERROR, as in Clojure -- it used to invent a tagged literal for anything, so a typo read as a good value. The registry is not built: `deps.edn` binds tag NAME to reader VAR, per project and applying only to that project's roots, so two libraries wanting `#x` no longer collide. The reader REWRITES `#x form` to `(var form)` and runs nothing; printing resolves the var back to whatever name this build bound it to |
