...
| [0034](0034-tagged-literals.md) | A tagged literal is a value, not a map | **Queued.** The reader builds a two-key map, which is ambiguous with a map in every format that has tags, loses the namespace in JSON, and makes `0033`'s reserved-tag guard a shape heuristic instead of a name check |
