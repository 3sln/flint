...
// --- the host's half of the port protocol (`doc/decisions/0027`) -----------
//
// A run that came back 2 is parked on the host. The host answers with these,
// then calls `flint_resume`. None of them re-enters the interpreter: they
// RECORD, and the answer is acted on at the next pump -- because a host may
// well call one from inside a host function the runtime itself invoked, and
// re-entering there would run the scheduler on top of itself.

/// Answer an open-request. `ok == 0` refuses it. Returns 0 when the token is
/// stale or already used, which is exactly the late-or-duplicated reply that
/// would otherwise resume a stranger's thread.
#[no_mangle]
pub extern "C" fn flint_host_continue(token: u32, ok: u32) -> u32 {
    unsafe {
        let rt = ensure_rt();
...
/// Push bytes into the flint end of a host port. Returns 0 when the guest's
/// buffer is full and the host must offer the message again after the next
/// pump -- inbound needs the same back-pressure as outbound.
#[no_mangle]
pub extern "C" fn flint_host_deliver(port_id: u32, ptr: u32, len: u32) -> u32 {
    unsafe {
        let rt = ensure_rt();
        let bytes = core::slice::from_raw_parts(ptr as *const u8, len as usize);
        rt.host_deliver(port_id as i64, bytes) as u32
    }
}

/// The host lets go of its end.
#[no_mangle]
pub extern "C" fn flint_host_close_port(port_id: u32) {
    unsafe {
        let rt = ensure_rt();
        rt.host_close_port(port_id as i64);
    }
}

/// What state the RUNTIME end of this port is in. 255 means the runtime knows
/// nothing about this id, which a host should also treat as done: it makes the
/// pushed `:closed` an optimisation over polling rather than the sole carrier
/// of the truth.
#[no_mangle]
pub extern "C" fn flint_host_port_state(port_id: u32) -> u32 {
    unsafe {
        let rt = ensure_rt();
        rt.host_port_state(port_id as i64) as u32
    }
}

/// Serialise every pending event into `OUT` and answer how many there are.
/// Read them with `out_ptr`/`out_len`. Layout: `count` records of five
/// little-endian `u32`s -- `kind, a, b, payload-offset, payload-len` --
/// followed by the payload bytes, offsets relative to the start of the buffer.
#[no_mangle]
pub extern "C" fn flint_drain_events() -> u32 {
    unsafe {
        let rt = ensure_rt();
...
/// Re-enter the scheduler after answering. The same status codes as
/// `flint_main`: 0 finished, 1 threw, 2 parked on the host again.
#[no_mangle]
