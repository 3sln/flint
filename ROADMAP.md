...
| AOT on the JVM and CLR artifacts | **load-time AOT shipped 2026-09-25**; ahead-of-time not started | both emitters already existed (`AotEmit.java` 408 lines, `AotEmit.cs` 498) and were reached only from test drivers: `boot` never consulted the image's `FLAG_PERF`, so `:optimize [perf]` produced an artifact interpreted exactly like one without it. Now measured by ENTRIES into compiled code — jvm 0 → 961, clr 0 → 1997 — with the zero-entries control asserted beside it in `bin/check-four-ops` and `bin/check-clr`. Ahead-of-time (compiling into the artifact at build time) still wants `TypeSpec`/generic signatures in the CLR writer |
...
| No compile-time linking: one prebuilt runtime per target, compiler embeds it as data, `flint compile wasm`/`wasm-aot` splices | in progress | [`no-runtime-linking`](DECISIONS.md#no-runtime-linking) — splice + tree-shaker (57–79% of `wasm-ld`'s result, no linker) done; byte strings done (the blocker); **JVM/CLR embedded targets SHIPPED 2026-09-25**: `:to :clr` and `:to :jvm` both emit a self-contained artifact carrying the bytecode as constant data, through `bin/flint` AND the native CLI, gated by `test/selfhost-targets.clj` on the artifact's magic bytes. `:to :llvm` gained the same `boot`/`loop`/`link` face, so all four targets now expose it (`bin/check-artifact-ops`, 4 of 4) |
